...
- Koehler, E., Brown, E. & Haneuse, S. (2009). "On the Assessment of Monte Carlo Error in Simulation-Based Statistical Analyses." The American Statistician.

### Data-Driven Nests
- Almagro, M., Lai, K. & Manresa, E. (2025). "Data-Driven Nests." Working paper, Chicago Booth.
- Bonhomme, S. & Manresa, E. (2015). "Grouped Patterns of Heterogeneity in Panel Models." Econometrica.
- Fosgerau, M., Monardo, J. & de Palma, A. (2024). Misspecification bias from wrong nests.
...
- **Prefer supply-side-informed data-driven nests.** Instead of pure demand-side clustering, merge supply-side categories (e.g., license types, brand families) using demand-pattern similarity. This preserves the exclusion restriction while letting the data optimize the grouping.
- **Use enough nests for IV variation.** With K=4 nests, n_in_nest barely varies (all nests are huge). With K=48, there is rich variation. The sweet spot is typically K=8-15 for platforms with ~50K+ products.
- **Compute nest-specific sigma_k.** A single sigma can hide substantial heterogeneity. In AI model markets, sigma_k ranges from 0.63 (niche categories) to 0.99 (mainstream categories). Report sigma_k as a separate empirical finding about market segmentation.
- **Run the full pipeline for EVERY specification.** When comparing nesting structures, ensure all diagnostics (placebo tests, causal analysis, bootstrap) are computed for each spec. Do not leave blank cells in comparison tables.
- **Validate with Monte Carlo FIRST, then compare specifications.** The MC study should use a DGP calibrated to the real data structure (same M, similar J, same instrument set). Only after MC confirms the estimation strategy is sound should you compare specifications.
- **Do not use the Hansen J test to choose among nesting structures** (see Section 17). Use economic plausibility, placebo lead tests, parameter reasonableness (validated via MC), and parsimony. The Hansen J has excessive power in large samples and is unreliable with nonlinear GMM due to criterion function non-convexity.

### Don't Do That
- **Don't cluster on endogenous demand outcomes (downloads, trending scores, demand residuals) and then use count instruments.** The count instruments (n_in_nest, n_outside_nest) require supply-side nest definitions. Demand-side clustering creates corr(n_in_nest, log_dl) < 0, which inverts the first stage and pushes sigma to boundary values (0 or negative).
- **Don't use too few nests (K=4-6) with count instruments on large markets.** Each nest will contain 10K-40K products, making n_in_nest nearly constant across markets.
- **Don't assume the Almagro method works out-of-the-box in every setting.** It requires time-varying exogenous product characteristics for identification. If your market lacks these (e.g., free digital products with no price variation), the reduced-form clustering step may not produce nests that are useful for structural estimation.
- **Don't report nest-specific sigma_k without explaining the economic interpretation.** High sigma_k (>0.9) means products in the nest are near-perfect substitutes; low sigma_k (<0.5) means products are differentiated. The pattern across nests tells a story about market segmentation.
...
## 22. SUPERVISOR FEEDBACK PATTERNS AND HOW TO RESPOND

### Key Principle
Experienced supervisors (professors, thesis advisors) give feedback that often has deeper methodological implications than the surface-level request. Learning to decode supervisor feedback and respond with the right combination of code, results, and writing is essential for productive research collaboration.

### Common Feedback Patterns

**"I recommend you ignore the Hansen J test."**
- Surface: Don't use J as a model selection criterion.
- Deeper meaning: (1) The Hansen J has excessive power in large samples — with millions of observations, it will reject ANY slightly misspecified model. (2) In nonlinear GMM, the J statistic at a local minimum may not reflect the global J. (3) Model selection should be based on ECONOMIC reasoning (do the nests make sense?), MONTE CARLO validation (does the estimator work in controlled settings?), and PLACEBO tests (do the instruments pass exogeneity checks?). (4) Reporting J is fine as a diagnostic, but framing it as "the J passes/fails" to justify model choice is a red flag.
- How to respond: Replace J-based model selection with a multi-criteria approach: (a) sigma interior and significant, (b) beta in MC-calibrated range, (c) placebo lead test passes, (d) economic plausibility of nests. Present all specifications transparently and explain why one is preferred on economic grounds.

**"Try running a Monte Carlo study."**
- Surface: Validate your estimation strategy with simulated data.
- Deeper meaning: (1) Nonlinear GMM can have non-convexity issues — MC checks whether your optimizer finds the right answer. (2) The professor wants to see that you understand your own estimator's properties. (3) This is a non-negotiable best practice for structural papers.
- How to respond: Design the MC DGP to match your actual data (same M, similar J, same endogeneity structure, same instruments). Use 500+ replications (Paxton et al. 2001; Koehler et al. 2009 recommend 2,500+ for precise coverage). Report bias, RMSE, and 95% CI coverage. Show the sampling distribution in histograms.
- Literature: Paxton et al. (2001) recommend 500 minimum; Koehler et al. (2009) show 2,500 needed for coverage within 1 unit; 1,000 is the most common in published work; Conlon & Gortmaker (2020) use MC extensively.

**"I don't understand what your natural experiment teaches us."**
- Surface: The motivation for the reduced-form/DiD section is unclear.
- Deeper meaning: (1) In a structural paper, the structural model already has a causal interpretation (through IV). So "proving causality" with a DiD is redundant at best, undermining at worst ("if you need a DiD, your structural model isn't identified"). (2) The DiD should be framed as: (a) OUT-OF-SAMPLE VALIDATION — the structural model predicts X, does the natural experiment confirm it? (b) POLICY QUANTIFICATION — the natural experiment measures the effect of a specific, implementable design change. (c) ROBUSTNESS — the DiD coefficient should be similar across different structural specifications.
- How to respond: Rewrite the motivation to say: "The IV strategy already addresses endogeneity; the natural experiment is useful because it provides an out-of-sample check on the model's predictions and quantifies the effect of a specific platform design change."

**"What is [technical test X]?"**
- Surface: The professor doesn't understand a diagnostic you reported.
- Deeper meaning: If the professor doesn't understand it, neither will referees. You need to explain it more clearly.
- How to respond: Add 2-3 sentences of intuitive explanation RIGHT AFTER introducing the test. Don't assume the reader knows what a "placebo lead test" or "Anderson-Rubin CI" is. Explain the test, then report the result.

**"Can you run these tests at the [broader] level?"**
- Surface: Test whether your finding generalizes beyond the specific subsample.
- Deeper meaning: (1) A finding that holds only in one subsample is fragile. (2) Running the same analysis across multiple subsamples (different pipelines, different time periods, different model categories) provides a form of external validity.
- How to respond: Run the analysis across all relevant subsamples and present the results in a single figure or table. Emphasize whether the qualitative finding generalizes even if the magnitudes differ.

### The Completeness Principle
**Every diagnostic, test, and analysis should be computed for ALL specifications you compare, not just the preferred one.** A comparison table with blank cells signals cherry-picking. If a test cannot be run for a given specification (e.g., placebo lead test is not applicable to the no-search model), explain why in a footnote rather than leaving the cell empty.

---

## COMPLETE REFERENCE LIST

(Original references preserved, new additions below)

### Data-Driven Nests
- Almagro, M., Lai, K. & Manresa, E. (2025). "Data-Driven Nests." Working paper, Chicago Booth & NBER.
- Bonhomme, S. & Manresa, E. (2015). "Grouped Patterns of Heterogeneity in Panel Models." Econometrica.
- Fosgerau, M., Monardo, J. & de Palma, A. (2024). Misspecification bias from wrong nests.

### Monte Carlo Methods (expanded)
- Advani, A., Kitagawa, T. & Sloczynski, T. (2019). "Mostly Harmless Simulations?" JAE.
- Paxton, P. et al. (2001). "Monte Carlo Experiments: Design and Implementation." SEM.
